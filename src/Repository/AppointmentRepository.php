...
class AppointmentRepository extends Repository{
    ...
    function getAll(){
         $rows = $this->db->fetchAll('SELECT *, appointments.uuid AS uuid FROM appointments LEFT JOIN clients ON appointments.client_uuid = clients.uuid');
         $appointments = [];
    ...
    function getAppointmentWithClientInfo($uuid){
        $rows = $this->db->fetchAll('SELECT *, clients.uuid as "clientUuid", appointments.uuid as "appointmentUuid" FROM appointments LEFT JOIN clients ON appointments.client_uuid=clients.uuid WHERE appointments.uuid = ?', [$uuid]);
    ...
    function update($post){
        ...
        if($post['client_uuid']){
          $query->set('client_uuid', $qb->expr()->literal($post['client_uuid']));
        }
